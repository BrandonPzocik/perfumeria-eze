...
router.patch("/", requireAuth, (req, res) => {
  ...
  db.prepare(
    `UPDATE settings SET
      whatsapp_number=?, whatsapp_message=?, store_name=?, store_name_accent=?,
      logo_url=?, banner_url=?, primary_color=?, accent_color=?,
      instagram_url=?, facebook_url=?, schedule=?, currency=?, show_currency=?, dark_mode_default=?
    WHERE id = 1`
  ).run(
    merged.whatsapp_number, merged.whatsapp_message, merged.store_name, merged.store_name_accent,
    merged.logo_url, merged.banner_url, merged.primary_color, merged.accent_color,
    merged.instagram_url, merged.facebook_url, merged.schedule, merged.currency, merged.show_currency, merged.dark_mode_default
  );
